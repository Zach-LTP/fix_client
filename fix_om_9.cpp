...
public:
    ...
    void fromApp(const FIX::Message& message, const FIX::SessionID& sessionID) override {
        ...
        if (msgType == FIX::MsgType_ExecutionReport)
        {
            ...
            if (executionReport.getField(FIX::FIELD::OrdStatus) == std::string(&FIX::OrdStatus_NEW, 1) && !first_access)
            {
                ...
                replaceRequest.set(FIX::Price(std::stod(executionReport.getField(FIX::FIELD::Price))));
                replaceRequest.set(FIX::OrderQty(std::stod(executionReport.getField(FIX::FIELD::OrderQty))));
