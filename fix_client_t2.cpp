        ...
        try {
            ...
            if (msgTypeValue == FIX::MsgType_MarketDataSnapshotFullRefresh || msgTypeValue == FIX::MsgType_MarketDataIncrementalRefresh) {
                ...
                while (true) {
                    ...
                    if (message.isSetField(FIX::FIELD::MsgSeqNum)) {
                        FIX::StringField seqNumField(FIX::FIELD::MsgSeqNum);
                        message.getField(seqNumField);
                        seqNum = seqNumField.getString();
                    }
                    ...
                    std::cout << "Timestamp=[" << timestamp << "], SeqNo=[" << seqNum << "] Entry " << index << ": Symbol=" << symbol << ", Type=" << entryType << " (" << entryTypeDesc << "), Price=" << price << ", Volume=" << volume << std::endl;
            ...
            for (FIX::FieldMap::const_iterator it = headerFields.begin(); it != headerFields.end(); ++it) {
                ...
                std::cout << "Field " << tag << " (" << description << "): " << it->getString() << std::endl;
            }
