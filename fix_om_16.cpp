...
public:
    ...
    void onLogon(const FIX::SessionID& sessionID) override {
        ...
        order_id = "ltp"+std::to_string(time(NULL))+"ltp";
        order.set(FIX::ClOrdID(order_id));
    ...
    void fromApp(const FIX::Message& message, const FIX::SessionID& sessionID) override {
        ...
        if (msgType == FIX::MsgType_ExecutionReport) {
           ...
           if (ordStatus == FIX::OrdStatus_NEW) {
                ...
                FIX44::NewOrderSingle order2;
                order2.set(FIX::ClOrdID(order_id));
                order2.set(FIX::Price(100000));
                order2.set(FIX::OrderQty(0.0002));
                order2.set(FIX::Symbol("BTCUSD"));
                order2.set(FIX::Side(FIX::Side_BUY));
                order2.set(FIX::OrdType(FIX::OrdType_LIMIT));
                // order.set(FIX::TimeInForce(FIX::TimeInForce_IMMEDIATE_OR_CANCEL));
                order2.set(FIX::TimeInForce(FIX::TimeInForce_GOOD_TILL_CANCEL));
                order2.set(FIX::Account("4ab47a03-9625-4430-8a3d-e2d35ee7d0b4"));
                order2.set(FIX::TransactTime(time(NULL)));
                FIX::Session::sendToTarget(order2, sessionID);

                // 发送 NewOrderSingle 消息
                std::cout << "Sending NewOrderSingle: " << order2 << std::endl;
                FIX::Session::sendToTarget(order2, sessionID);
